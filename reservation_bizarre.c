...
//=====================================================
// LES TYPES STRUCT DE LA GESTION DES PLACES
// struct siege {
//   int  numero     ;
//   char position   ;
...
// struct salle {
//   char   position ;
//   struct siege *tab_place ;
// };

// struct wagon {
//   int classe ;
//   int type ;
//   struct salle *tab_salle ;
// };

// struct rame {
//   char type[5] ;
//   struct wagon *tab_rame ;
// } ;
...
//   2/ de nuancer les types de rames de TGV */
//   char dep_gare[MAXstrNOMGARE] ;
//   char arr_gare[MAXstrNOMGARE] ;
//   int  heure_dep               ;
//   int  heure_arr               ;
...
int nbvoyage;
struct UnVoyage *tab_voyages ; // variable tableau de struct voyage (= les places réservables)
...
// FIN DES TYPES STRUCT DE LA GESTION DES PLACES
//=====================================================
...
void chargement_place() ;
void chargement_place3() ;
void chargement_place4() ;
void chargement_place5() ;
void suppression_places5() ;
void creation_places5() ;
void ListerRep() ;
...
void chargement_horaires()
{
  ...
  //f1=fopen("./data_sncf/sncf_full_test.txt", "r") ;
  // f1=fopen("./data_sncf/sncf_full.csv","r") ;
  f1=fopen("./data/horaire/horaires_tgv_ter.csv","r") ;
...
// ~~~~~~~~~~~
void chargement_place2()
{
  char nom_rep[100] ;
  char *list_objet ;
  strcpy(nom_rep, "./data/place") ;
  ListerRep(nom_rep, &list_objet) ;
}

void ListerRep(char nom_rep[], char *list_objet)
{
  // char  *dir_name="./data";
  DIR *d1 ;              /* descripteur du répertoire */
  struct dirent *objet ; /* une entrée dans le répertoire */
  int i, cpt=0;
  // static char list_objet[3000][100] ;

  d1=opendir(nom_rep) ;
  if(d1 == NULL)
  {
    fprintf(stderr, "Erreur d'accès à '%s':%s\n",nom_rep,strerror(errno));
    exit(EXIT_FAILURE);
  }
  else
  {
    do
    {
      ...
      { 
        if ((strcmp(objet->d_name,".")!=0)&&(strcmp(objet->d_name,"..")!=0))
        {
          ...
          strcpy(list_objet[cpt++],objet->d_name);
  ...
  // return list_objet ;
    ...
    for (i=2;i<cpt;i++)
    {
      printf("valeur de list_trajet : %s\n",list_objet[i]) ;
    }
  }
} 
...
void chargement_place5() // version 1 fichier 2D - 1 structure 2D
{
  ...
  int id, date ;
  char type[10], dep_gare[MAXstrNOMGARE], arr_gare[MAXstrNOMGARE] ;
  int hd=0, ha=0, nbsalle=0, wagon=0, classe=0, salle=0, siege=0, position=0, etat=0, billet=0;
  ...
  /*--- Ouverture fichier horaires --- */
  //f1=fopen("./data_sncf/sncf_full_test.txt", "r") ;
  f1=fopen("./data/places.txt","r") ;
  i=nbvoyage ;
  while ((! feof(f1)))
  {
    fscanf(f1,"%d;%d;",&id,&date);
    tab_voyages2[i].id = id ;
    tab_voyages2[i].date = date ;

    // lecture de la 3e colonne
    j=0;
    retour=0;
    while((retour!=59) &&(retour!=10)) // tant que different de LF et de ";"
    {
    ...
    while((retour!=59) &&(retour!=10)) // tant que different de LF et de ";"
    {
      ...
      if((retour!=59) &&(retour!=10))  // si different de LF et de ";"
      {
        dep_gare[j++] = lettre  ;          // insertion
      }
      type[j] = '\0' ;                 // cloture de la chaine
    }

    // lecture de la 5e colonne
    j=0;
    retour=0;
    while((retour!=59) &&(retour!=10)) // tant que different de LF et de ";"
    {
  ...
  // printf("Ici ok 2, %c \n",lettre)  ;
      ...
      if((retour!=59) &&(retour!=10))  // si different de LF et de ";"
      {
        arr_gare[j++] = lettre  ;          // insertion
      }
      type[j] = '\0' ;                 // cloture de la chaine
    }

    fscanf(f1,"%d;%d;%d;%d;%d;%d;%d;%d;%d;%d",
      &hd, &ha, &nbsalle, &wagon, &classe,
      &salle, &siege, &position, &etat, &billet) ;
    // tab_voyages2[i].id = id ;
    // tab_voyages2[i].date = date ;
    strcpy(tab_voyages2[i].type,type);
    strcpy(tab_voyages2[i].dep_gare,dep_gare);
    strcpy(tab_voyages2[i].arr_gare,arr_gare);
    tab_voyages2[i].hd = hd ;
    tab_voyages2[i].ha = ha ;
  ...
  // printf("%d %d %s %s %s %d %d %d %d %d %d %d %d %d %d\n",tab_voyages2[i].id,tab_voyages2[i].date,
  //   tab_voyages2[i].type,tab_voyages2[i].dep_gare,tab_voyages2[i].arr_gare,tab_voyages2[i].hd,tab_voyages2[i].ha,
  //   tab_voyages2[i].nbsalle,tab_voyages2[i].wagon,tab_voyages2[i].classe,tab_voyages2[i].salle,tab_voyages2[i].siege,tab_voyages2[i].position,
  //   tab_voyages2[i].etat,tab_voyages2[i].billet);
    i++ ;
    nbvoyage = i ;
...
void suppression_places5()
{
  printf("nombre lus : %d\n",nbvoyage) ;
  int i ;
  for (i=0;i<nbvoyage-1;i++)
  {
    if (tab_voyages2[i].date<tab_date[0].date)
    {
      tab_voyages2[i] = tab_voyages2[i+1] ;
    }
    nbvoyage-- ;
  }
...
void creation_places5()
{
  ...
  int *tab_id ;
  int nbid ;
  int *tab_datepresentedoublon;
  int nbdatepresentedoublon ;
  int *tab_datepresenteunique;
  int nbdatepresenteunique ;
  int *tab_datemanquante;
  int existe;

  // on parcourt tab_horaire pour récupérer les id de trajet distincts
  for (i=0;i<nbhoraire;i++)
  {
    tab_id = (int *) malloc(sizeof(int));
    if (tab_horaires[i].id != tab_horaires[i+1].id)
    {
      tab_id[j] = tab_horaires[i].id ;
      nbid++;
      tab_id = (int *) realloc(tab_id,sizeof(int) * (nbid+1));
    }
  }

  // pour chaque id de trajet distinct
  for (i=0;i<nbid;i++)
  {
    // on parcourt les voyages pour trouver 
    for (j=0;j<nbvoyage;j++)
    {
      // s'il y a des voyages de cet id
      if (tab_id[i] == tab_voyages2[j].id)
      {
        // on parcourt les dates ouvertes à la vente
        existe=0;
        for (k=0;k<nbdatevente;k++)
        {
          // si pour ce voyage, il existe des places pour cette date ouverte
          if (tab_date[k].date == tab_voyages2[j].date)
          {
            existe=1;
          }
        }
      ...
      // s'il n'en existe pas on crée les places
      if (existe==0)
      {
            // selon le type
            if (strcmp(tab_voyages2[j].type,"TGV")==0)
            {

            }
          }

  //         // tab_datepresentedoublon = (int *) malloc(sizeof(int)) ;
  //         // // si la date est déjà présente (= toutes les places du train ont été ouvertes)
  //         // if (tab_voyages2[j].date == tab_date[k].date)
  //         // {
  //         //   // on retient la date presente dans un tableau
  //         //   tab_datepresentedoublon[l] = tab_date[k].date ;         
  //         //   l++ ;
  //         //   tab_datepresentedoublon = (int *) realloc(tab_datepresentedoublon,sizeof(int) * (l+1)) ;
  //         // }
  //         // nbdatepresentedoublon = l ;
  //       }
  //       // on garde un exemplaire unique des dates presentes
  //       for (l=0;l<nbdatepresentedoublon-1;m++)
  //       {
  //         if (tab_datepresentedoublon[l] != tab_datepresentedoublon[l+1])
  //         {
  //           tab_datepresenteunique = (int *) malloc(sizeof(int)) ;
  //           tab_datepresenteunique[m] = tab_datepresentedoublon[l];
  //           m++ ;
  //           tab_datepresenteunique = (int *) realloc(tab_datepresenteunique,sizeof(int) * (m+1)) ;
  //         }
  //       }
  //       nbdatepresenteunique = m ;
  //       free(tab_datepresentedoublon);
  //       tab_datepresentedoublon=NULL;
  //       // on récupère les dates manquantes
  //       for (k=0;k<nbdatevente;k++)
  //       {
  //         for (l=0;l<nbdatepresenteunique;l++)
  //         {
  //           // if (tab_date[k]==)
  //         }
  //       }
      ...
      // on parcourt tab_voyages pour récupérer ceux qui ont le même id
    
      // tab_place[j+1].id = tab_horaire[i].id ;
      // tab_place[j+1].date = tab_date[i].id ;
  //    free(tabeleves) ; /* suppression du bloc de données */
  //    tabeleves=NULL ; /*on réinitialisae le pointeur à NULL*/
//}
...
void crea_date(int jour, int mois, int annee)
{
  ...
  {
    tab_date[i].jhebdo=calcul_jour_semaine(j,m,a,jour_sys,mois_sys,annee_sys,jhebdo_num_sys) ;
    tab_date[i].jour=j;
    tab_date[i].mois=m;
    tab_date[i].annee=a;
    ...
    chdate[7]=(j%10)+48;
    chdate[8]='\0';
    // strcpy(tab_date[i].date,chdate); // je remplace par un int, c'est plus simple à traiter
    date = atoi(chdate) ;
 ...
 // printf("%d\n", tab_date[i].date);
    ...
    i++;
    nbdatevente=i;
    tab_date = (struct date *) realloc(tab_date,sizeof(struct date) * (nbdatevente+1)) ;
