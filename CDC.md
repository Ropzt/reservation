...
* *date[] -> : un tableau des dates pour lesquelles ce trajet circule
    ...
    * *sequence[] -> : un tableau des sequences du trajet 
        * `stop_seq`: le numéro de séquence
        * *places[] -> : un tableau des places (disponibilité et caractéristiques) pour cette séquence
            * `wagon`
            * `classe`
            * `salle`
            * `siege`
...
* La procédure convertie chaque date en une donnée de type `integer` au format AAAAMMJJ et la range dans un tableau

#### Chargement des données Places

* La procédure commence par compter le nombre de dossier présent dans `./data/place` :
    * Si il n'y en a aucun, elle crée tous les dossiers `./data/place/<idtrajet>`
    * Si il en existe, elle crée les éventuels dossier manquants

* La procédure lie le fichier `places.txt` de référence
* La procédure vérifie, pour chaque date, pour chaque trajet si il circule à cette date :
...
* La procédure vérifie ensuite, pour chaque date pour chaque trajet, si il existe des dossiers `./data/place/<idtrajet>/<date>/<séquence>` et crée les éventuels dossiers manquants

* La procédure vérifie ensuite, pour chaque séquence pour chaque date pour chaque trajet, si il existe des fichiers `./data/place/<idtrajet>/<date>/<séquence>/places.txt`
    * Si non, elle les crée en utilisant le fichier `places.txt` de référence

* La procédure lie alors le fichier et le charge dans la structure contenant les places 
